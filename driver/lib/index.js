...
exports.saveUserIntents = function(userId, intents) {
    ...
    for(var room in intents) {
        ...
        // updates.push(
            // db['rooms.intents'].update({room}, { $set: { [`users.${userId}.objects`]: intents[room]}}, {upsert: true}));
        updates.push(
...
exports.getRoomIntents = function(roomId) {
    ...
    .then(data=>{
        ...
        //db['rooms.intents'].findOne({room: roomId});
};
...
exports.clearRoomIntents = function(roomId) {
    return env.del(env.keys.ROOM_INTENTS+roomId)//db['rooms.intents'].removeWhere({room: roomId});
};
